...
def _normalize_device_keys(d: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(d, dict):
        return {}
    dl = { (k.lower() if isinstance(k, str) else k): v for k, v in d.items() }
    name = dl.get('name') or dl.get('meter_name') or dl.get('device_name') or ''
    addr_raw = (
        dl.get('address') if 'address' in dl else dl.get('meter_address') if 'meter_address' in dl else dl.get('device_id')
    )
    try:
        address = int(addr_raw) if addr_raw is not None else None
    except Exception:
        address = None
    model = dl.get('model') or dl.get('meter_model') or dl.get('type') or ''
    location = dl.get('location') or dl.get('site') or dl.get('plant') or 'Unknown'
    merged = dict(d)
    if name:
        merged['name'] = name
