...
# Prefer the centralized `offline-setup-12Sep` folder as authoritative for the
# usb copy script. If that folder exists next to this repo, use it for the
# systemd ExecStart so only that copy is executed by the service.
...
[INFO] Installing USB auto-copy service
...
[Service]
Type=simple
WorkingDirectory=${PRIMARY_DIR}
ExecStart=${PY_EXEC} ${PRIMARY_DIR}/usb_csv_auto_copy.py --daemon
Restart=on-failure
RestartSec=5
Nice=10
